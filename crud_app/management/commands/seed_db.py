...
class Command(BaseCommand):
    help = 'Populates the database with a specified number of random records for testing.'
    ...
    def handle(self, *args, **options):
        ...
        proveedores = [Proveedor.objects.create(nombre=fake.company(), created_by=random.choice(users)) for _ in range(number)]
        clientes = [Cliente.objects.create(nombre=fake.name(), created_by=random.choice(users)) for _ in range(number)]
        ...
        productos = [
            Producto.objects.create(
                ...
                precio_compra_actual=fake.pydecimal(left_digits=3, right_digits=2, positive=True, min_value=1, max_value=500),
        ...
        for _ in range(num_compras):
            ...
            Compra.objects.create(
                ...
                precio_compra_unitario=fake.pydecimal(left_digits=3, right_digits=2, positive=True, min_value=1, max_value=450),
                created_by=random.choice(users)
            )
            ...
            try:
                ...
                if producto_venta.stock > 0:
                    ...
                    Venta.objects.create(
                        ...
                        precio_venta=fake.pydecimal(left_digits=4, right_digits=2, positive=True, min_value=int(producto_venta.precio_compra_actual) + 1, max_value=9999),
                        created_by=random.choice(users)
                    )
        ...
        productos_a_restaurar = random.sample(productos_a_borrar, min(len(productos_a_borrar), 2))
