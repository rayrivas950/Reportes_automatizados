...
from .serializers_reconciliation import (
    VentaImportadaReconSerializer,
    CompraImportadaReconSerializer,
)
...
class VentaImportadaViewSet(viewsets.ModelViewSet):
    ...
    http_method_names = [
        ...
        "head",
        "options",
    ]  # Solo permitir GET y POST para la acción
    ...
    def procesar(self, request, pk=None):
        ...
        if venta_importada.estado not in [
            VentaImportada.Estados.PENDIENTE,
        ...
        producto = Producto.objects.filter(
            nombre__iexact=venta_importada.producto_nombre
        ).first()
        if not producto:
            conflictos["producto"] = (
                f"El producto '{venta_importada.producto_nombre}' no existe."
            )

        cliente = Cliente.objects.filter(
            nombre__iexact=venta_importada.cliente_nombre
        ).first()
        if not cliente:
            conflictos["cliente"] = (
                f"El cliente '{venta_importada.cliente_nombre}' no existe."
            )
        ...
        try:
            ...
            venta_importada.save()
            return Response(
                {"mensaje": "Venta procesada y creada exitosamente."},
                status=status.HTTP_200_OK,
            )
        except Exception as e:
            return Response(
                {"error": f"Error al crear la venta final: {str(e)}"},
...
class CompraImportadaViewSet(viewsets.ModelViewSet):
    ...
    def procesar(self, request, pk=None):
        ...
        if compra_importada.estado not in [
            CompraImportada.Estados.PENDIENTE,
        ...
        conflictos = {}
        producto = Producto.objects.filter(
            nombre__iexact=compra_importada.producto_nombre
        ).first()
        if not producto:
            conflictos["producto"] = (
                f"El producto '{compra_importada.producto_nombre}' no existe."
            )

        proveedor = Proveedor.objects.filter(
            nombre__iexact=compra_importada.proveedor_nombre
        ).first()
        if not proveedor:
            conflictos["proveedor"] = (
                f"El proveedor '{compra_importada.proveedor_nombre}' no existe."
            )
        ...
        try:
            ...
            compra_importada.save()
            return Response(
                {"mensaje": "Compra procesada y creada exitosamente."},
                status=status.HTTP_200_OK,
            )
        except Exception as e:
            return Response(
                {"error": f"Error al crear la compra final: {str(e)}"},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
