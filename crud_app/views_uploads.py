...
def validar_y_normalizar_columnas(df_original, mapeo_columnas):
    ...
    for col in columnas_excel:
        ...
        if col_lower in mapeo_inverso:
            ...
            if canonico in canonicos_encontrados:
                ...
                return None, f"El archivo contiene columnas duplicadas semánticamente para '{canonico}'."
    ...
    if columnas_faltantes:
        return None, f"Faltan las siguientes columnas requeridas: {', '.join(columnas_faltantes)}."
...
class VentaUploadView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            df_normalizado, error = validar_y_normalizar_columnas(df_original, self.MAPEO_COLUMNAS_VENTA)
            if error:
...
class CompraUploadView(APIView):
    ...
    MAPEO_COLUMNAS_COMPRA = {
        ...
        'cantidad': ['cantidad', 'unidades'],
        'precio_compra_unitario': ['precio', 'costo', 'precio_compra', 'precio unitario', 'valor'],
    }
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            df_normalizado, error = validar_y_normalizar_columnas(df_original, self.MAPEO_COLUMNAS_COMPRA)
            if error:
                return Response({"error": error}, status=status.HTTP_400_BAD_REQUEST)
            ...
            for index, row_normalizada in df_normalizado.iterrows():
                ...
                if serializer.is_valid():
                    compra_importada = CompraImportada(
                        ...
                        cantidad=str(serializer.validated_data.get('cantidad')),
                        precio_compra_unitario=str(serializer.validated_data.get('precio_compra_unitario'))
                    )
                    ...
                    compra_importada = CompraImportada(
                        ...
                        cantidad=str(row_normalizada.get('cantidad', '')),
                        precio_compra_unitario=str(row_normalizada.get('precio_compra_unitario', ''))
                    )
                    compras_a_crear.append(compra_importada)
